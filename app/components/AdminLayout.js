...
export default function AdminLayout({ children, title = 'Dashboard' }) {
    ...
    const timer = setTimeout(async () => {
      try {
        const [p, m, c, t] = await Promise.all([
          ...
          fetch('https://zeex-digital-production.up.railway.app/api/clients').then(r => r.json()),
  ...
  const fetchNotifCount = async () => {
    try {
      const r = await fetch(`https://zeex-digital-production.up.railway.app/api/notifications?user_id=${user?.id || 1}`);
  ...
  const markNotifRead = async (id) => {
    try {
      await fetch(`https://zeex-digital-production.up.railway.app/api/notifications/${id}/read`, { method: 'PUT' });
