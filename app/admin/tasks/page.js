...
export default function Tasks() {
  ...
  const openTaskDetail = async (task) => {
    ...
    try {
      const [sR, cR] = await Promise.all([
        fetch(`http://localhost:5000/api/tasks/${task.id}/subtasks`),
        fetch(`http://localhost:5000/api/tasks/${task.id}/comments`),
      ]);
  ...
  const addSubtask = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/tasks/${openTask.id}/subtasks`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const toggleSubtask = async (sub) => {
    ...
    try {
      await fetch(`http://localhost:5000/api/subtasks/${sub.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const deleteSubtask = async (id) => {
    try {
      await fetch(`http://localhost:5000/api/subtasks/${id}`, { method: 'DELETE' });
  ...
  const addComment = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/tasks/${openTask.id}/comments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
