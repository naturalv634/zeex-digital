...
export default function InvoicesPage() {
  ...
  const openEdit = async (inv) => {
    try {
      const full = await fetch(`http://localhost:5000/api/invoices/${inv.id}`).then(r => r.json());
      setEditId(inv.id);
      setForm({
  ...
  const handleSave = async () => {
    ...
    const url = editId ? `http://localhost:5000/api/invoices/${editId}` : 'http://localhost:5000/api/invoices';
  ...
  const handleMarkPaid = async (inv) => {
    await fetch(`http://localhost:5000/api/invoices/${inv.id}`, {
