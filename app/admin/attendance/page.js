...
export default function AttendancePage() {
  ...
  const fetchToday = async () => {
    setLoading(true);
    try {
      const r = await fetch('http://localhost:5000/api/attendance/today');
  ...
  const fetchHistory = async () => {
    setLoading(true);
    try {
      const r = await fetch(`http://localhost:5000/api/attendance?date=${filterDate}`);
  ...
  const fetchMonthly = async () => {
    setLoading(true);
    try {
      const r = await fetch(`http://localhost:5000/api/attendance/monthly?month=${monthlyMonth}`);
                ...
                <tbody>
                  {historyRows.map((r, i) => (
                    <tr key={r.id} style={{ borderBottom: i < historyRows.length - 1 ? `1px solid ${C.border}` : 'none' }}
                      ...
                      <td style={{ padding: '14px 18px' }}>
                        {r.photo_path ? (
                          <img src={`http://localhost:5000${r.photo_path}`} alt="selfie" style={{ width: 38, height: 38, borderRadius: '50%', objectFit: 'cover', border: `2px solid ${C.green}`, cursor: 'pointer' }}
                            onClick={() => window.open(`http://localhost:5000${r.photo_path}`, '_blank')} />
